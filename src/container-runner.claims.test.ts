...
import { abortGatewaySessionObservers, adoptRunningSessions, isContainerRunning, killContainer } from './container-runner.js';
import { type GatewaySessionInput, resetGatewayProvider } from './gateway-providers/index.js';
const release = vi.fn(async () => {});
const ensure = vi.fn(async (_input: GatewaySessionInput) => ({ contribution: { networkAccess: { endpoint: 'http://proxy:8080', target: { kind: 'runtime' as const, identity: 'proxy' } } }, release }));
...
beforeEach(async () => {
  ...
  resetGatewayProvider({ kind: 'test-lifecycle', agentSkills: [], sessions: { ensure, reapOrphans }, approvals: { subscribe: async () => {} } });
...
describe('session claim lifecycle', () => {
  ...
  it('detaches when a runtime stop fails, instead of revoking a potentially live identity', async () => {
    const handle = fakeHandle('sess-1', 'container-a');
    handle.stop = async () => { throw new Error('runtime unavailable'); };
    snapshots.push({ handle, phase: 'running' } as SupervisedSnapshot);
    await adoptRunningSessions();
    killContainer('sess-1', 'test-stop');
    await vi.waitFor(() => expect(release).toHaveBeenCalledWith({ kind: 'host-detached', reason: 'test-stop' }));
  });
