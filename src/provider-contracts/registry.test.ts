...
describe('provider model destinations', () => {
  it('derives endpoint URLs and network destinations from a new provider declaration', () => {
    registerProviderHostContract('model-destination-fixture', {
      ...emptyContract(), modelDomains: ['models.example.test'],
  ...
  it.each(['https://evil.test', 'http://models.example.test', 'https://user:secret@models.example.test',
    'https://models.example.test:8443', 'https://models.example.test/?token=secret'])('rejects an endpoint outside the declared HTTPS boundary: %s', (api) => {
    expect(() => registerProviderHostContract('invalid-model-endpoint', {
      ...emptyContract(), modelDomains: ['models.example.test'], modelEndpoints: { api },
    })).toThrow();
  });
});
