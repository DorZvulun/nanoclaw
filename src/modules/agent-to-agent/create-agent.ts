...
async function performCreateAgent(
  ...
  // Subagent path: a child inherits its creator's EFFECTIVE provider, model and
  // effort, NOT the instance-wide default — so a child is never spawned on a
  // runtime the parent can't reach (e.g. a codex-only install where claude
  // isn't authenticated), nor on a model or reasoning setting the parent
  // deliberately left. The provider goes to initGroupFilesystem, which pins the
  // child's scaffold and stamps its config row in one step (a NULL parent
  // resolves to claude); model and effort are stamped after, and only when the
  // parent set them, so an unset parent leaves the child on the defaults. The
  ...
  const inheritedRuntime = {
    ...(parentConfig?.model && { model: parentConfig.model }),
    ...(parentConfig?.effort && { effort: parentConfig.effort }),
  };
  if (Object.keys(inheritedRuntime).length > 0) {
    await updateContainerConfigScalars(newGroup.id, inheritedRuntime);
  }
