...
async function processGatewayRequest(
  ...
  try {
    if (!controller || controller.signal.aborted || unavailable || providerKind !== selectedKind)
    ...
    if (request.trigger === 'default' && request.destination && session) {
      if (permitsConfiguredGatewayRead(request.destination)) return 'approve';
      ...
      if (domains.some((domain) => host === domain || host.endsWith(`.${domain}`))) return 'approve';
    }
...
async function expireApproval(
  approvalId: string,
  reason: 'no response' | 'host restarted' | 'bridge failed',
): Promise<void> {
...
async function sweepStaleGatewayApprovals(legacyActions: readonly string[]): Promise<void> {
  if (approvalSource?.listPending) {
    try {
      ...
      if (approvalSource.durable) {
        const live = new Set(snapshot.map((request) => request.id));
