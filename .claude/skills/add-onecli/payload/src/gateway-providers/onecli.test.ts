...
it.each(compatibilityFixtures)('preserves native OneCLI approval content: $name', async (fixture) => {
  ...
  await sdk.manualApproval!({
    id: 'native-fixture', createdAt: new Date(Date.now() + 1000).toISOString(),
    expiresAt: new Date(Date.now() + 30000).toISOString(), ...fixture.request,
    summary: fixture.summary, agent: { name: 'Nano', externalId: 'g1' },
  });
  expect(decide.mock.calls[0][0].summary).toEqual({
    agent: 'Nano', action: fixture.summary.action, details: fixture.summary.details,
